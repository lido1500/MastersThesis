...
partitions permet d'utiliser plusieurs vecteurs de \DIFdelbegin \DIFdel{mouvement }\DIFdelend \DIFaddbegin \DIFadd{mouvements }\DIFaddend (un par partition)
pour mieux modéliser la surface. Cependant, les vecteurs de \DIFdelbegin \DIFdel{mouvement
}\DIFdelend \DIFaddbegin \DIFadd{mouvements
}\DIFaddend supplémentaires doivent, eux aussi, être encodés, ce qui décourage l'usage
inutile de petites partitions.

Lorsque le nombre de partitions est élevé, encoder les vecteurs de \DIFdelbegin \DIFdel{mouvement }\DIFdelend \DIFaddbegin \DIFadd{mouvements }\DIFaddend de
chacune d'elles peut engendrer un coût binaire considérable et particulièrement
à bas débit. C'est pourquoi, dans la norme H.264, les vecteurs de \DIFdelbegin \DIFdel{mouvement
}\DIFdelend \DIFaddbegin \DIFadd{mouvements
}\DIFaddend sont encodés différentiellement à l'aide de la médiane ou d'une prédiction
...
corrélation entre les vecteurs de \DIFdelbegin \DIFdel{mouvement }\DIFdelend \DIFaddbegin \DIFadd{mouvements }\DIFaddend de blocs avoisinants. Les blocs
...
l'encodage. Ceci explique pourquoi la recherche de vecteurs de \DIFdelbegin \DIFdel{mouvement }\DIFdelend \DIFaddbegin \DIFadd{mouvements }\DIFaddend est
...
\DIFdelbeginFL %DIFDELCMD < \LT{Refaire la \fig{fig-4x4PredictionModes} en $3 \times 3$, ajouter 'd'}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \LT{Refaire la \fig{fig-4x4PredictionModes} en $3 \times 3$}
...
(diagonale \DIFdelbegin \DIFdel{bas-droit}\DIFdelend \DIFaddbegin \DIFadd{sud-est}\DIFaddend ). On obtient $\mathbf{d}$ à l'aide de la formule suivante~:
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \SC{on ne voit pas 'd' sur la figure. Aussi on y parle de bas-droit et pas sud-est.}
...
\hline
Prédiction \DIFdelbeginFL \DIFdelFL{\textit{intra} }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Intra }\DIFaddendFL et en bordure de macrobloc & 4 (Lissage fort)\\
Prédiction \DIFdelbeginFL \DIFdelFL{\textit{intra} }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Intra }\DIFaddendFL & 3 \\
Prédiction \DIFdelbeginFL \DIFdelFL{\textit{inter} }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Inter }\DIFaddendFL et présence de résiduel & 2 \\
Différence des vecteurs de mouvement $\geqslant$ 1 & 1 \\
Prédiction \DIFdelbeginFL \DIFdelFL{\textit{inter} }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Inter }\DIFaddendFL avec des trames de références différentes& 1 \\
Sinon & 0 (Aucun lissage) \\
\hline
\end{tabular}
\end{table}

Comme le démontre \DIFdelbegin \DIFdel{le tableau}\DIFdelend \DIFaddbegin \DIFadd{la table}\DIFaddend ~\ref{tab-FilteringRules}, le type de
